/* ============================================================
   Spring Internals — engine dùng chung (vanilla, không backend)
   - esc/hl/renderCode/setupTabs  (port từ design-pattern)
   - STEP ENGINE   : animation từng bước + tô sáng dòng code
   - QUIZ + PROGRESS: mượn ý tưởng từ project "study" (localStorage)
   ============================================================ */
(function () {
  "use strict";
  var KEY = "spring-internals-progress-v1";
  ...
  // tô màu 1 dòng code Java (annotation / keyword / number / method / string / comment)
  function hl(line) {
    var c = line.indexOf("//");
    var code = line, comment = "";
    if (c !== -1) { code = line.slice(0, c); comment = line.slice(c); }
    var out = esc(code)
      .replace(/(@[A-Za-z]\w*)/g, '<span class="an">$1</span>')
      .replace(/\b(public|private|protected|static|final|void|new|return|class|interface|extends|implements|this|super|if|else|for|while|throw|throws|try|catch|import|package|null|true|false|abstract|enum)\b/g,
  ...
  function renderLesson() {
    ...
    // thanh tiến độ nhỏ trên trang bài
  ...
  function applyStep(n) {
    ...
    // chuyển tab code nếu cần + tô sáng dòng
  ...
  window.SI = { esc: esc, hl: hl, renderCode: renderCode };
})();
